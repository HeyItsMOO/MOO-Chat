...
description: Add ChatMOO to your Shopify storefront with the theme app block — no code required.
order: 4
---

ChatMOO installs on Shopify as a **theme app block**, so you can add it from the theme editor without touching code.

## Steps

1. In the ChatMOO dashboard, open **Install** and copy your **public key** (`moo_xxxxxxxx`).
2. In Shopify, go to **Online Store → Themes → Customize**.
3. Open **App embeds** (or add the **ChatMOO** block from the `integrations/shopify` extension).
4. Enable the ChatMOO block and paste your public key.
...
- **Block not showing in the editor?** Make sure the ChatMOO extension is installed for your store.
